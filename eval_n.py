...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/home/farhan/attMPTI/blocks_bs1_s1',
                        help='Directory to the source data')
    parser.add_argument('--ova_data_path', type=str, default='/home/farhan/attMPTI/OVA_Dataset/',
                        help='Directory to the source data')
    ...
    for batch_idx, (data, sampled_classes, xyz_min) in enumerate(test_loader):
        [support_x, support_y, query_x, query_label] = data
        print(support_x.shape, support_y.shape, query_x.shape, query_label.shape)
        if torch.cuda.is_available():
            data = cast_cuda(data)
        # print('query label shape', query_label.shape)
        if args.phase == 'protoeval':
            ...
            learner = MPTILearner(args, mode='test')
        # print("????????",args.ova_data_path)
        # print("===========",data[3].shape, data[2].shape, data[1].shape, data[0].shape)
        ######## Denormalisation######
        # print("Shape of block 1",data[2][0].shape)
        # print("Shape of block 2",data[2][1].shape)
        # quer_d = query_x.cpu().detach().numpy()
        
        # block_1 = np.transpose(quer_d[0])
        # block_2 = np.transpose(quer_d[1])
        # xyz_1 = block_1[:, 0:3]
        # rgb_1 = block_1[:, 3:6]
        # xyz_min_1 = np.amin(xyz_1, axis=0)
        # xyz_1 += xyz_min_1
        # xyz_2 = block_2[:, 0:3]
        # rgb_2 = block_2[:, 3:6]
        # xyz_min_2 = np.amin(xyz_2, axis=0)
        # xyz_2 += xyz_min_2
        # # if 'XYZ' in 'xyz':
        # #     xyz_min_1 = np.amin(xyz_1, axis=0)
            
        # #     xyz_max_1 = np.amax(XYZ, axis=0)
        # #     XYZ_1 = XYZ_1*xyz_max_1
        # #     XYZ = xyz_1 + xyz_min_1
        # block_1[:, 0:3]= xyz_1              
        # block_2[:, 0:3]= xyz_2
        # quer_d[0][0:3,:] = block_1[:, 0:3].T
        # quer_d[1][0:3,:] = block_2[:, 0:3].T
        # quer_d= query_x
        ...
        for q in range(query_point.shape[0]):
            ...
            query_point_dn= query_point[q,0:3,:].T+xyz_min[q]
            #y_label = query_pred[q, :]
            query_block = np.concatenate((query_point_dn[q, :,:], y_label), axis=0)
            query_scene.append(query_block.T)
    #         print(query_scene[0].shape)
    # print(query_scene)
    query_scene = np.vstack(query_scene)
    np.save(os.path.join('/home/farhan/attMPTI/Output','Scene.npy'), query_scene)
